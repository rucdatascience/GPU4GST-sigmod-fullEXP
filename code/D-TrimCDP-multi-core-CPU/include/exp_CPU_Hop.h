...
void exp_CPU_Hop(string path, string data_name, int T, int D, int task_start_num, int task_end_num)
{
	...
	outputFile.open(path+"result/D-TrimCDP-multi-core-CPU_" + data_name + "_T" + to_string(T) + "_"+"D"+to_string(D)+"_" + to_string(task_start_num) + "-" + to_string(task_end_num) + ".csv");
	...
	//for (int i = task_end_num; i >= task_start_num; i--)
	{
		...
		if (1)
		{
				...
				records ret;
				graph_hash_of_mixed_weighted solu = HOP_MT(generated_group_vertices_hash, v_generated_group_graph, v_instance_graph, D, time_record,RAM,ret);
