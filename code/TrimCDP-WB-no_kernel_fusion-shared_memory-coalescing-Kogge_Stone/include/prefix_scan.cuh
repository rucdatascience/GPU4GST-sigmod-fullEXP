...
__forceinline__ __device__ void warp_scan_inc(data_t &value, const index_t laneId) 
{
	...
	value +=__shfl_up_sync(0xffffffff,value, 1, 32)*(laneId >= 1);
	value +=__shfl_up_sync(0xffffffff,value, 2, 32)*(laneId >= 2);
	value +=__shfl_up_sync(0xffffffff,value, 4, 32)*(laneId >= 4);
	value +=__shfl_up_sync(0xffffffff,value, 8, 32)*(laneId >= 8);
	value +=__shfl_up_sync(0xffffffff,value, 16, 32)*(laneId >= 16);
}
...
__forceinline__ __device__ void _grid_scan(
		...
		const data_t input, //输入的是位值
		data_t &output,//返回的是前缀和
		data_t *smem,
