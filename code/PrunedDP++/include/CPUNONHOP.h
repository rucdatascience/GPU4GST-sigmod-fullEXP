...
graph_hash_of_mixed_weighted graph_v_of_v_idealID_PrunedDPPlusPlus(graph_v_of_v_idealID &input_graph, graph_v_of_v_idealID &group_graph, std::unordered_set<int> &cumpulsory_group_vertices, int maximum_return_app_ratio, int &RAM_MB, double &time_record, records &ret)
{
			...
			for (auto it = non_overlapped_group_sets_IDs[p1].begin(); it != ite5; it++)
			{
				...
				{ // only merge optimal T(v,p2), this is recorded in the pseudo code of 2016 paper
					...
					{ // O(3^|Gamma||V| comparisons in totel, see the DPBF paper)
						...
						if (merged_tree_cost <= best_cost * 0.667 + error_safeguard)
