...
int main(int args, char **argv)
{
	...
	*ginst = new graph<long, long, long, vertex_t, index_t, weight_t>(file_beg_pos, file_adj_list, file_weight_list);
	...
	outputFile.open(path+"result/TrimCDP-WB-kernel_fusion-no_shared_memory_prefix_scan-global_memory_coalescing_" + data_name + "_T" + to_string(T) + "_" + to_string(task_start_num) + "-" + to_string(task_end_num) + ".csv");
