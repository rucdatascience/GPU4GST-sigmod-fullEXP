...
//__shared__ vertex_t smem[32]; // 32位共享内存
	...
	while (true)
	{
		...
		compute_mapper.mapper_push_coalesced(
			mdata.worklist_sz_lrg[0],
			mdata.worklist_lrg,
			mdata.cat_thd_count_lrg,
			blockIdx.x,	 /*group id*/
...
collect_work(
	...
	int *work_size_sml,
	int *work_size_mid,
	int *work_size_lrg, int init_flag)
{
	const index_t TID = threadIdx.x + blockIdx.x * blockDim.x;
	const index_t GRNTY = blockDim.x * gridDim.x;
	const index_t tid_in_wrp = threadIdx.x & 31;   // 线程在warp的位置
	...
	Barrier global_barrier)
{
	...
	assert(blk_size * grd_size <= BLKS_NUM * THDS_NUM);
	int a[3] = {0, 0, 0};
	int *work_size_sml = nullptr, *work_size_mid = nullptr, *work_size_lrg = nullptr;
	int *work_size_sml_host = &a[0];
	int *work_size_mid_host = &a[1];
	int *work_size_lrg_host = &a[2];
	cudaMalloc((void **)&work_size_sml, sizeof(int));
	cudaMalloc((void **)&work_size_mid, sizeof(int));
	cudaMalloc((void **)&work_size_lrg, sizeof(int));
	// cudaFuncGetAttributes
	...
	collect_work<<<grd_size, blk_size>>>(level,
										 ...
										 compute_mapper,
										 worklist_gather,
										 global_barrier, work_size_sml, work_size_mid, work_size_lrg, 1);
	cudaDeviceSynchronize();
	// cout << "size " << *work_size_sml << " " << *work_size_mid << " " << *work_size_lrg << endl;
	cudaMemcpy(work_size_sml_host, work_size_sml, sizeof(int), cudaMemcpyDeviceToHost);
	cudaMemcpy(work_size_mid_host, work_size_mid, sizeof(int), cudaMemcpyDeviceToHost);
	cudaMemcpy(work_size_lrg_host, work_size_lrg, sizeof(int), cudaMemcpyDeviceToHost);
	work_size = *work_size_sml_host + *work_size_mid_host + *work_size_lrg_host;
	cout << "size " << *work_size_sml_host << " " << *work_size_mid_host << " " << *work_size_lrg_host << endl;
	while (work_size != 0)
	{
		rounds++;
		balanced_push_kernel_small<<<grd_size, blk_size>>>(level,
														   ggraph,
														   mdata,
														   compute_mapper,
														   worklist_gather,
		...
		collect_work<<<grd_size, blk_size>>>(level,
											 ...
											 compute_mapper,
											 worklist_gather,
											 global_barrier, work_size_sml, work_size_mid, work_size_lrg, 0);
		cudaDeviceSynchronize();
		cudaMemcpy(work_size_sml_host, work_size_sml, sizeof(int), cudaMemcpyDeviceToHost);
		cudaMemcpy(work_size_mid_host, work_size_mid, sizeof(int), cudaMemcpyDeviceToHost);
		cudaMemcpy(work_size_lrg_host, work_size_lrg, sizeof(int), cudaMemcpyDeviceToHost);
