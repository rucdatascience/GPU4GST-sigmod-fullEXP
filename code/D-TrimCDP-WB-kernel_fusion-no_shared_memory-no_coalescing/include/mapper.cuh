...
public:
	...
	__forceinline__ __device__ void
	mapper_push_coalesced(		 // 这个是大中小都使用的push函数 这里会进行更新新的距离
		index_t wqueue, // 这个是本轮的用值
		index_t *worklist,
		index_t *cat_thd_count, // 这个是本轮返回值
		const index_t GRP_ID,
		...
		feature_t level)
	{
		...
		for (index_t i = GRP_ID; i < WSZ; i += GRP_COUNT)
		{
			...
			{
				int total = end - beg;
				int chunk = (total + GRP_SZ - 1) / GRP_SZ; // 向上取整，保证所有边都能被分配
				int start = beg + THD_OFF * chunk;
				int stop = min(end, start + chunk);
	...
	__forceinline__ __device__ void
	mapper_push_new(		 // 这个是大中小都使用的push函数 这里会进行更新新的距离
		index_t wqueue, // 这个是本轮的用值
		index_t *worklist,
		index_t *cat_thd_count, // 这个是本轮返回值
		const index_t GRP_ID,
